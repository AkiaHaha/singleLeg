...
{
////////////////////////////////mt////////////////////////////////////////////////////////////////
auto mt::prepareNrt()->void
{

   for(auto &m:motorOptions()) m = aris::plan::Plan::NOT_CHECK_ENABLE;
}
auto mt::executeRT()->int
{
   ...
   TCurve t1(0.5, 0.2);
   t1.getCurveParam();
   controller()->motionPool()[0].setTargetPos(begin_angle + t1.getTCurve(count()));
   return t1.getTc()*1000 - count();

}
mt::mt(const std::string &name)
{
...
auto MoveJoint::executeRT()->int
{
    ...
    TCurve s1(5,2);
    s1.getCurveParam();
    double pos1 = begin_pos[0] + dir_  * PI* s1.getTCurve(count());
    double pos2 = begin_pos[1] + dir_*2  * PI* s1.getTCurve(count());
//    if (count() % 200)
//    {
...
//    }
    ...
    controller()->motionPool()[0].setTargetPos(pos1);
    controller()->motionPool()[1].setTargetPos(pos2);

    // read toeque //
    std::int16_t torque=0;
    this->ecController()->motionPool()[0].readPdo(0x6077,0x00,torque);
    this->ecController()->motionPool()[1].readPdo(0x6077,0x00,torque);
//    mout()<< torque <<std::endl;
...

{
    ...
    controller()->motionPool()[0].setTargetToq(100);
    return 1;
...
auto createControllerQuadruped()->std::unique_ptr<aris::control::Controller>
{
    ...
    for (aris::Size i = 0; i < 2; ++i)
    {
        ...
        {
//            16384.0 * 6.4 / 2 / PI
            131072.0*10 / 2 / PI,131072.0*10 / 2 / PI
        };
        double max_pos[12]
        {
            PI,PI
